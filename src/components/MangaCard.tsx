    ...
    >
        <Link href={`/manga/${manga.id}`} className="block">
          ...
          <div className="relative aspect-[2/3] overflow-hidden bg-[var(--mist)]">
            {imgError ? (
              <div className="absolute inset-0 flex items-center justify-center bg-[var(--mist)]">
                <span className="text-[var(--stone)] text-sm text-center px-4">{manga.title}</span>
              </div>
            ) : (
              <Image
                src={manga.image}
          ...
          <motion.button
            ...
            className="absolute bottom-4 left-4 right-4 py-3 bg-[var(--paper)] text-[var(--ink)] text-xs tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            onClick={(e) => {
              e.preventDefault();
              addToCart(manga);
            }}
